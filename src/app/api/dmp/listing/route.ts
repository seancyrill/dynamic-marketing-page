...
export async function POST(request: NextRequest) {
  try {
    const { VIDEO_SERVER_CONVERT } = process.env
    if (!VIDEO_SERVER_CONVERT) {
      throw new Error(`Cannot read video server env`)
    }
    ...
    if (unconvertedImages.length) {
      ...
      const response = await fetch(VIDEO_SERVER_CONVERT, {
        method: "POST",
